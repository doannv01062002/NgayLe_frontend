...
import { FlashSaleSession, FlashSaleProduct } from "@/types/home-management";
...
export default function AdminFlashSalesPage() {
  ...
  // Product Config Modal State (replacing prompt)
  ...
  const fetchSessions = async () => {
    try {
      const data = await flashSaleService.getAllSessions();
  ...
  const handleCreateSession = async () => {
    try {
      await flashSaleService.createSession(newSessionData);
  ...
  const handleToggle = async (id: number) => {
    try {
      await flashSaleService.toggleSession(id);
      if (managingSession?.sessionId === id) {
        // refresh detail if open
        const updated = await flashSaleService.getSession(id);
        setManagingSession(updated);
      }
  ...
  const handleDelete = (id: number) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          await flashSaleService.deleteSession(id);
          fetchSessions();
  ...
  const openManageModal = async (session: FlashSaleSession) => {
    try {
      // Fetch fresh details to get products
      const details = await flashSaleService.getSession(session.sessionId);
      setManagingSession(details);
    } catch (e) {
      error("Không thể tải chi tiết phiên");
    }
  };

  // Called when a product is selected from picker
  const handleProductSelect = (product: Product) => {
    setProductConfig({
      isOpen: true,
      product: product,
      flashSalePrice: product.basePrice, // default
      quantity: 1,
    });
    // Keep picker open? No, close it or keep underlying?
    // Usually close picker to focus on config
    // setIsProductPickerOpen(false); // Let's close picker first
    // Actually, maybe we want to keep picker open to add multiple?
    // Let's close picker for clarity for now.
  };

  const submitAddProduct = async () => {
    if (!managingSession || !productConfig.product) return;

    try {
      await flashSaleService.addProducts(managingSession.sessionId, [
        {
      ...
      setIsProductPickerOpen(false); // Close picker now if not already
      setProductConfig({ ...productConfig, isOpen: false });

      // Refresh details
      const updated = await flashSaleService.getSession(
  ...
  const handleRemoveProduct = (fspId: number) => {
    setConfirmModal({
      isOpen: true,
      title: "Xóa sản phẩm",
      ...
      onConfirm: async () => {
        ...
        try {
          await flashSaleService.removeProduct(fspId);
          const updated = await flashSaleService.getSession(
            managingSession.sessionId
          );
          setManagingSession(updated);
          success("Đã xóa sản phẩm khỏi phiên");
        } catch (e) {
  ...
  return (
    <div className="p-6 w-full max-w-[1600px] mx-auto space-y-8">
      <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
        ...
        <span className="text-gray-500 font-medium">Marketing & Nội dung</span>
        <span className="text-gray-300 dark:text-gray-600">/</span>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-gray-500 dark:text-gray-400 max-w-2xl">
            Tạo và quản lý các khung giờ giảm giá sốc. Theo dõi hiệu quả các
            phiên bán hàng.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Đang diễn ra",
            value: sessions.filter((s) => s.isActive && s.status === "ONGOING")
              .length,
            icon: "flash_on",
          ...
          <div className="flex gap-2 pb-4">
            <div className="relative group">
              ...
              <input
                ...
                className="pl-10 pr-4 h-10 w-64 rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-800 text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all"
              />
            </div>
            <button className="h-10 px-4 border border-dashed border-gray-300 rounded-lg flex items-center gap-2 text-gray-500 hover:border-gray-400 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="mt-auto pt-4 border-t border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all shadow-sm active:scale-95 border ${
                            session.isActive
                              ? "bg-white text-red-600 border-red-200 hover:bg-red-50"
                              : "bg-white text-green-600 border-green-200 hover:bg-green-50"
                          }`}
                        >
                          {session.isActive ? "Dừng" : "Kích hoạt"}
                        </button>
                        <button
      ...
      {/* Product Config Modal (Price & Quantity) */}
